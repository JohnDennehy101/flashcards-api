...
# Prompt to enter a password for the PostgreSQL flashcards user (rather than hard-coding
# a password in this script).
read -p "Enter password for flashcards DB user: " DB_PASSWORD
...
# Set up the flashcards DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE flashcards"
sudo -i -u postgres psql -d flashcards -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d flashcards -c "CREATE ROLE flashcards WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the flashcards database to the system-wide environment
# variables in the /etc/environment file.
echo "FLASHCARDS_DB_DSN='postgres://flashcards:${DB_PASSWORD}@localhost/flashcards'" >> /etc/environment
